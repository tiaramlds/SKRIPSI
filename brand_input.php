...
if (isset($_POST['simpan'])) {
  ...
  if (empty($error)) {
    if ($foto_name) {
      ...
      $foto_name = "brand_" . time() . "_" . $foto_name;
      move_uploaded_file($foto_file, $direktori . "/" . $foto_name);

      $foto = $foto_name;
    } else {
      $foto_name = $foto; //memasukkan data dari data yg sudah ada sebelumnya
    }

    if ($id != "") {
      $sql1  = "update brand set nama = '$nama', foto = '$foto_name', isi= '$isi', tgl_isi=now() where id = '$id'";
    } else {
      $sql1       = "insert into brand (nama,foto, isi) values ('$nama','$foto_name', '$isi')";
    }
...
?>
<h1>Halaman Admin Input Data Brand</h1>
<div class="mb-3 row">
  <a href="brand.php">
    << Kembali ke halaman admin brand </a>
</div>
<?php
if ($error) {
?>
